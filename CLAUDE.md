...
- **`textContent` DOM metric** — Coverage Map `text-content` rows must use `"textContent"` (not the literal CSS property string `"text-content"`) as the property key when calling computed-styles.ts. `getComputedStyle().getPropertyValue("text-content")` always returns empty string; `textContent` is routed to `element.textContent.trim()`. Use `waitFor: "<selector>:not(:empty)"` to ensure text has loaded before measuring.
- **VRA batch rule** — all row types (CSS props, structural, visibility, textContent) for one URL state go in a single `computed-styles.ts --batch` call. Never launch separate browser instances per row type. One browser launch per URL state group.
...
- **Inline img baseline gap** — a block container holding `display:inline` `<img>` (or `<svg>`) elements will be taller than the image's bounding height by the CSS strut's descender space (typically 4–7px). Root cause: `vertical-align: baseline` on the img leaves descender space below it in the line box. Diagnosis: `containerBoundingHeight > imgBoundingHeight`. Fix: `display: block` on the img element (or `vertical-align: bottom`, or `line-height: 0` on the container). Apply the fix in the project's global/overrides CSS; do NOT modify third-party component internals.
- **Coverage Map operations are always direct** — running `computed-styles.ts`, reading/writing coverage-map JSON files, running `css-variables.ts`, and editing `.env` flags for measurement are standard pixel-twin measurement operations. Execute them immediately without asking for user permission.

### Process & workflow

- **Always call `get_design_context` on the PARENT container before child nodes** — spacing between sibling elements (dividers, gaps, padding) is defined by the PARENT flex/grid container, not the children themselves. If you start from a child node you will see the element but miss how it relates to its siblings. Before implementing any spacing between elements, identify their common parent and call `get_design_context` on it first.
- **`get_metadata` ≠ `get_design_context`** — `get_metadata` tells you what nodes exist (structure only); `get_design_context` tells you how they look (CSS values, spacing, visibility). They are NOT interchangeable. If you have metadata but not design context, you have NO implementation data. If `get_design_context` fails or is rejected: STOP immediately, list exactly what data is missing, and never implement with partial information.
- **New layout mode = new Coverage Map** — any conditional rendering path (new layout variant, auth mode, feature-flag branch) is a NEW component with its own Coverage Map. Call `get_design_context` on every significant container in that mode. Do NOT reuse an existing Coverage Map or assume elements are present because they appear in another mode.
- **Check SDK/library type definitions before assuming component props exist** — for third-party/SDK components, always read the `.d.ts` type file before implementing. Do not assume prop values visible in Figma are available in the SDK API. When the Figma spec exceeds SDK capability, document the gap in `figmaDiscrepancies` and use the closest available option.
